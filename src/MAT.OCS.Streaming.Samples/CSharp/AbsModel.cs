...
{
    public class AbsModel : KafkaTopicModeler
    {
        private const double Frequency = 100; // Hz

        /// <inheritdoc />
        public AbsModel(string brokerList, string inputTopicName, string outputTopciName)
        ...
        protected override DataFormat CreateOutputDataFormat()
        {
            return DataFormat.DefineFeed()
                .Parameters(new List<string> { "Abs(Sin(x)+Cos(2x))" })
        ...
        protected override TelemetryData ProcessData(TelemetryDataBuffer dataBuffer)
        {
            ...
            {
                parameter1.AvgValues[i] = Math.Abs(parameter1.AvgValues[i]);
            }
                            ...
                            {
                                {
                                    "group", new ParameterGroup
                                    {
                                        ...
                                        {
                                            {
                                                "Abs(Sin(x)+Cos(2x))", new Parameter
                                                {
                                                    Name = "Abs(Sin(x)+Cos(2x))",
                                                    PhysicalRange = new Range(0, Math.PI)
